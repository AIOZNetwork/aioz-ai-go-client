// Code generated by go-swagger; DO NOT EDIT.

package models

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// RequestGetOrganizationSpendingCostStatisticsRequest request get organization spending cost statistics request
//
// swagger:model request.GetOrganizationSpendingCostStatisticsRequest
type RequestGetOrganizationSpendingCostStatisticsRequest struct {

	// from
	// Example: 2023-05-07 15:04:05
	From string `json:"from,omitempty"`

	// to
	// Example: 2023-05-07 15:04:05
	To string `json:"to,omitempty"`
}

// Validate validates this request get organization spending cost statistics request
func (m *RequestGetOrganizationSpendingCostStatisticsRequest) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this request get organization spending cost statistics request based on context it is used
func (m *RequestGetOrganizationSpendingCostStatisticsRequest) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *RequestGetOrganizationSpendingCostStatisticsRequest) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *RequestGetOrganizationSpendingCostStatisticsRequest) UnmarshalBinary(b []byte) error {
	var res RequestGetOrganizationSpendingCostStatisticsRequest
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
